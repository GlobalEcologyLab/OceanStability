...
## RCP
r45 <- mask(rast(file.path(outDir, "outputs/rcp45_rasters_ocean_warming.grd"))[["stSNR_ocn_warm"]], 
            mask = ocean_mask)
r85 <-  mask(rast(file.path(outDir, "outputs/rcp85_rasters_ocean_warming.grd"))[["stSNR_ocn_warm"]], 
            mask = ocean_mask)
rcp_snr <- rast(list(r45, r85))
names(rcp_snr) <- c("RCP45", "RCP85")

## Paleo
paleo_snr <- mask(rast(file.path(outDir, "outputs/paleo_rasters_ocean_warming.grd"))[["stSNR_ocn_warm"]], 
            mask = ocean_mask)
...
```{r plotSNR, echo = TRUE, fig.show='asis', fig.cap="SNR values for the past and two future scenarios", fig.align='center', fig.height=5, fig.width=9}
...
# Export to CSV
fwrite(reg_SNR, file.path(outDir, "outputs/SNR_Cell.csv"))
```
...
```{r SNRcomparison, fig.show='asis', fig.cap="SNR values within each region, standardised across time. n = number of cells in each region.", fig.align='center', fig.height=12, fig.width=12, out.width='100%'}
...
```{r overlapHisto, fig.show='asis', fig.cap="Figure S2 - Density plots of SNR within each marine bioregion", fig.align='center', fig.height=12, fig.width=12, out.width='100%'}
exc_reg <- nCells[N < 5, ][["Region"]]
# Calculate median and quantiles for plotting
# Calculate Q10, 50, and 90 to add to plot
signif.lines <- reg_SNR[!Region %in% exc_reg, .(xMed = quantile(SNR, 0.5),
...
# Save results to CSV
fwrite(regional_overlap, file.path(outDir, "outputs/regional_overlap.csv"))
fwrite(regional_overlap, file.path(outDir, "outputs/TableS1.csv"))
...
st_write(regions, file.path(outDir, "outputs/marine_regions_SNR_overlap_poly.shp"), 
...
writeRaster(regions_overlap, 
            file.path(outDir, "outputs/marine_regions_SNR_overlap.grd"),
            overwrite = TRUE)
```

A quick plot of the overlap in each of the regions shows the following

```{r plotOV, fig.show='asis', fig.cap="Figure S3 - Plot of SNR overlap within each marine bioregion", fig.align='center', fig.height = 6.5, fig.width = 5, out.width='100%'}
...
ggplot() +
  ...
  scale_fill_stepsn(colours = hcl.colors(15, "Temps", rev = TRUE),
                       limits = c(0, 0.701),                       
                       breaks = scales::pretty_breaks(14)(seq(0, 0.7, length = 7)),
...
```{r makeFig2, fig.width=9, out.width='100%', results='markup', fig.show='hold'}
...
map02 <- ggplot() +
  ...
  scale_fill_stepsn(colours = cptcity::cpt("jjg_cbcont_div_cbcSpectral", 18)[-c(16:18)],
                    breaks = seq(0, 0.70, length = 15),
                    limits = c(-0.001, 0.701),
  ...
  scale_y_continuous(expand = c(0,0)) +
    geom_segment(data = signif.lines[Region == region & Scenario != "RCP85", ],
                 aes(y = 0.01, yend = 0.33, x = xMed, xend = xMed,
                     ...
                     arrow = arrow(ends = "first", angle = 30, length = unit(.25,"cm"))) +
  theme_void() +
...
walrus <- image_read(file.path(outDir, "Figures/Stock/walrus_small.jpg"))
manatee <- image_read(file.path(outDir, "Figures/Stock/manatee_small.jpg"))
reef <- image_read(file.path(outDir, "Figures/Stock/reef_small.jpg"))
hammer <- image_read(file.path(outDir, "Figures/Stock/hammerhead_small.jpg"))
...
fig02 <- ggdraw({{left_col} | {map02} | {right_col}} + 
  ...
  draw_line(x = c(0.17, 0.43),
            y = c(0.43, 0.43),
            size = 0.65,
            arrow = arrow(ends = "last", angle = 30, length = unit(.25,"cm"))) +
...
hs_temps <- setDT(readRDS(file.path(outDir, "Source/annual_temps_latbands_hotspots.RDS")))

trace <- ggplot(data = hs_temps[Scen == "TraCE" & Year < -100, ], 
                aes(x = Year/1000, y = lyr1, fill = value)) +
  geom_raster(na.rm = TRUE) +
  facet_wrap(~Scen, ncol = 1, labeller = as_labeller(c("TraCE" = "a"))) +
  scale_fill_stepsn(colours = cpt("arendal_temperature", n = 41),
                    limits = c(12, 32),
                    oob = scales::squish,
                    na.value = "transparent",
                    breaks = seq(12, 32, length = 21),
...
rcp <- ggplot(data = hs_temps[Scen != "TraCE", ], 
                aes(x = Year, y = lyr1, fill = value)) +
  facet_wrap(~Scen, ncol = 2, 
             labeller = as_labeller(c("RCP45" = "b", "RCP85" = "c"))) +
  geom_raster(na.rm = TRUE) +
  scale_fill_stepsn(colours = cpt("arendal_temperature", n = 21),
                    limits = c(12, 32),
                    oob = scales::squish,
                    na.value = "transparent",
                    breaks = seq(12, 32, length = 21),
...
```{r figS2, fig.width=9, out.width='100%', results='hide', fig.show='hold'}
# Projected bivariate map
