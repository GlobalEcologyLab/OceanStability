...
## RCP
r45 <- mask(rast(file.path(outDir, "rcp45_rasters_ocean_warming.grd"))[["stSNR_ocn_warm"]], 
            mask = ocean_mask)
r85 <-  mask(rast(file.path(outDir, "rcp85_rasters_ocean_warming.grd"))[["stSNR_ocn_warm"]], 
            mask = ocean_mask)
rcp_snr <- rast(list(r45, r85))
names(rcp_snr) <- c("RCP45", "RCP85")

## Paleo
paleo_snr <- mask(rast(file.path(outDir, "paleo_rasters_ocean_warming.grd"))[["stSNR_ocn_warm"]], 
            mask = ocean_mask)
...
# Export to CSV
fwrite(reg_SNR, file.path(outDir, "SNR_Cell.csv"))
```
...
# Save results to CSV
fwrite(regional_overlap, file.path(outDir, "regional_overlap.csv"))
fwrite(regional_overlap, file.path(outDir, "TableS1.csv"))
...
st_write(regions, file.path(outDir, "marine_regions_SNR_overlap_poly.shp"), 
...
writeRaster(regions_overlap, 
            file.path(outDir, "marine_regions_SNR_overlap.grd"),
            overwrite = TRUE)
```
