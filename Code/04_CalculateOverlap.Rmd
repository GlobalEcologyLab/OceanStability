...
hco_ts <- ggplot(data = hs_temps[Scen == "HCO" & mean >= 18, ], 
                aes(x = Year/1000, y = lyr1, fill = mean)) +
  geom_raster(na.rm = TRUE) +
  facet_wrap(~Scen, ncol = 1, scales = "free_x",
             labeller = as_labeller(c("HCO" = "b"))) +
  scale_fill_stepsn(colours = cpt("arendal_temperature", n = 24)[-(1:9)],
                    limits = c(18, 32),
                    oob = scales::squish,
                    na.value = "transparent",
                    breaks = seq(18, 32, length = 15),
                    labels = function(x) ifelse(x %% 2L == 0, x, "")) +
  scale_x_continuous(breaks = seq(-9,-5,length = 5), expand = c(0, 0),
                     limits = c(-9, -5)) +
  scale_y_continuous(limits = c(-45, 40), 
                     expand = c(0,0),
                     breaks = seq(-40,40,20)) +
...
hco_s <- ggplot(data = hs_temps[Scen == "HCO_Time" & value >= 18, ], 
                aes(x = Year/1000, y = lyr1, fill = value)) +
  geom_raster(na.rm = TRUE) +
  facet_wrap(~Scen, ncol = 1, scales = "free_x",
             labeller = as_labeller(c("HCO_Time" = "c"))) +
  scale_fill_stepsn(colours = cpt("arendal_temperature", n = 24)[-(1:9)],
                    limits = c(18, 32),
                    oob = scales::squish,
                    na.value = "transparent",
                    breaks = seq(18, 32, length = 15),
                    labels = function(x) ifelse(x %% 2L == 0, x, "")) +
  scale_x_continuous(expand = c(0, 0), breaks = seq(-7,-7,1),
                     labels = "") +
  scale_y_continuous(limits = c(-45, 40), 
                     expand = c(0,0),
                     breaks = seq(-40,40,20)) +
...
fig03 <- {trace}/{hco_ts + hco_s}/{rcp} + plot_layout(ncol = 1)
...
ggsave(filename = "Figure03.pdf", plot = fig03,
       device = cairo_pdf, path = "Figures",
       width = 9, height = 7, units = "in", dpi = 350)
...
ggsave(filename = "Figure03.png", plot = fig03,
       device = "png", type = "cairo", path = "Figures",
       bg = "white", width = 9, height = 7, units = "in", dpi = 350)
# plot
