...
knitr::opts_chunk$set(echo = TRUE, rows.print = 10, cols.print = 10,
                      ...
                      root.dir = "/mnt/d/Git/OceanStability")
```
...
outDir <- "/mnt/d/Git/OceanStability"

# Coastlines and aesthetics
coastlines <- ne_coastline(50, returnclass = "sf")
land <- ne_countries(50, returnclass = "sf")
oceans <- read_sf(file.path(outDir, "outputs/ne_50m_ocean.shp"))
st_agr(oceans) <- "constant"

ocean_mask <- read_sf(file.path(outDir, "outputs/ne_50m_ocean.shp"))
...
# hotspot data
rhs <- rast(file.path(outDir, "outputs/marine_species_hotspots_wgs84.tif"))
rhs

# hotspot polygons
hotspots <- read_sf(file.path(outDir, "outputs/marine_species_hotspots_poly.shp"))
...
hco <- rast(file.path(outDir, "source/trace21_HCO_biasCor.grd"))
...
xdf <- melt(setDT(
  as.data.frame(rast(list(hco[["HCO"]], rcp45, rcp85)),
...
writeRaster(analogue_dist,
            file.path(outDir, "outputs/climate_analogue_distances_wgs84.grd"),
            overwrite = TRUE)
...
writeRaster(analogue_dist_eez,
            file.path(outDir, "outputs/climate_analogue_distances_wgs84_eezAnalogues.grd"),
            overwrite = TRUE)
...
# write out and project for plotting. See below for why
write_sf(vectseez, file.path(outDir, "outputs/vects_eez.shp"))
#{plot(project(eez_mask, templ_ras, "ngb")); lines(vect(st_transform(vectseez, mollPrj)))}
