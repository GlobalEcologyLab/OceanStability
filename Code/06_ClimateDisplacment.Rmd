...
vect45 <- st_cast(rotate_prj(vect45, mollPrj), "LINESTRING")
# turn to data.table for plotting
## need to use geom_curve for nice plotting
vect45 <- data.table(x_start = st_coordinates(st_line_sample(vect45, sample = 0))[,1],
                     x_end = st_coordinates(st_line_sample(vect45, sample = 1))[,1],
                     y_start = st_coordinates(st_line_sample(vect45, sample = 0))[,2],
                     y_end = st_coordinates(st_line_sample(vect45, sample = 1))[,2],
                     Scen = "RCP45")
...
vect85 <- st_cast(rotate_prj(vect85, mollPrj), "LINESTRING")
# for plotting only.
vect85 <- data.table(x_start = st_coordinates(st_line_sample(vect85, sample = 0))[,1],
                     x_end = st_coordinates(st_line_sample(vect85, sample = 1))[,1],
                     y_start = st_coordinates(st_line_sample(vect85, sample = 0))[,2],
                     y_end = st_coordinates(st_line_sample(vect85, sample = 1))[,2],
                     Scen = "RCP85")

vects <- rbindlist(list(vect45, vect85))
# ranomdly sort the rows for plotting
## essentially so rcp85 doesnt dominate the plot!
{set.seed(54); vects <- vects[sample(1:nrow(vects), nrow(vects), replace = FALSE), ]}
...
ggplot() +
  ...
  scale_fill_gradientn(colours = cptcity::cpt("cmocean_curl", 100),
                       ...
                       labels = function(x) ifelse(test = (seq(-12000, 12000, by = 1000)/2000) %% 2 == 0, x, ""),
                       breaks = seq(-12000, 12000, by = 1000),
                       oob = scales::squish,
                       guide = guide_colourbar(
                         title = "Difference (RCP 8.5 - RCP 4.5; km)",
                         title.position = "top",
...
vect45eez <- st_wrap_dateline(st_cast(rotate_prj(vect45eez, mollPrj), "LINESTRING"))
# turn to data.table for plotting
## need to use geom_curve for nice plotting
vect45eez <- data.table(x_start = st_coordinates(st_line_sample(vect45eez, sample = 0))[,1],
                        x_end = st_coordinates(st_line_sample(vect45eez, sample = 1))[,1],
                        y_start = st_coordinates(st_line_sample(vect45eez, sample = 0))[,2],
                        y_end = st_coordinates(st_line_sample(vect45eez, sample = 1))[,2],
                        Scen = "RCP45")
...
vect85eez <- st_as_sf(gcIntermediate(dif85eez_int[, c("XS", "YS")],
                          ...
                          n = 100, addStartEnd = TRUE, 
                          breakAtDateLine = FALSE, sp = TRUE))
st_crs(vect85eez) <- "EPSG:4326"
vect85eez <- st_cast(rotate_prj(vect85eez, mollPrj), "LINESTRING")
vect85eez <- data.table(x_start = st_coordinates(st_line_sample(vect85eez, sample = 0))[,1],
                        x_end = st_coordinates(st_line_sample(vect85eez, sample = 1))[,1],
                        y_start = st_coordinates(st_line_sample(vect85eez, sample = 0))[,2],
                        y_end = st_coordinates(st_line_sample(vect85eez, sample = 1))[,2],
                        Scen = "RCP85")

vectseez <- rbindlist(list(vect45eez, vect85eez))
# ranomdly sort the rows for plotting
## essentially so rcp85 doesnt dominate the plot!
{set.seed(54); vectseez <- vectseez[sample(1:nrow(vectseez), nrow(vectseez), replace = FALSE), ]}
