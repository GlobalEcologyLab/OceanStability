...
# Trace
## stored as °C*100
## Users need to generate/source this themselves. Data is available in Paleoview
## but will require some post-processing to get to the same format
trace <- mask(rast(file.path(outDir, "Source/AnnualTemperatureStack_21000_0BP.nc")),
              mask = ocean_mask)
trace

# hotspot data
rhs <- rast(file.path(outDir, "marine_species_hotspots_wgs84.tif"))
rhs

# hotspot polygons
hotspots <- read_sf(file.path(outDir, "marine_species_hotspots_poly.shp"))
...
# Need to adjust trace temperatures with anomalies from SC data
sc_clim <- mask(rast(file.path(scPath, "ncdf/annual/StableClim_AnnMean_rcp45_ts.nc")), ocean_mask)
time(sc_clim) <- seq(as.Date("1850-06-16"), as.Date("2100-06-16"), by = "year")
sc_clim <- sc_clim[[which(as.integer(format(time(sc_clim), "%Y")) %between% c(1850,1950))]]
sc_clim <- app(sc_clim, mean)

# trace clim period
trace_clim <- trace[[which(-as.integer(gsub("ts_year=", "", names(trace))) >= -100)]]
trace_clim <- app(trace_clim, mean)/100

# Anomalies
anom <- sc_clim - trace_clim
anom

# add the anomalies to harmonise the trace data
trace <- (trace/100) + anom
```
...
hc_idx <- which(as.integer(gsub("ts_year=", "", names(trace))) %between% c(5000,9000))
hco <- app(trace[[hc_idx]], mean)
hco_sd <- app(trace[[hc_idx]], sd)
names(hco) <- "HCO"
names(hco_sd) <- "HCO_SD"
...
A quick plot of the averages shows warmer conditions by 2090 than that experienced during the Holocene Climatic Optimum.

```{r plotTemps, echo = TRUE, fig.show='asis', fig.cap="SST values during the Holocene Climatic Optimum and the future under RCP 4.5 and RCP 8.5", fig.align='center', fig.height=5, fig.width=9}
xdf <- melt(setDT(
  as.data.frame(rast(list(hco, rcp45, rcp85)),
                xy = TRUE)), id.vars = c("x", "y"))
xdf[, variable := factor(variable, levels = c("HCO", "RCP45", "RCP85"))]
ggplot() +
...
# RCP 45
worldSST_rcp45 <- as(rast(list(hco, rcp45, hco_sd)), "Raster")
names(worldSST_rcp45) <- c("MnSST_HCO", "MnSST_RCP45", "sdSST_HCO")
clim45 <- na.omit(data.frame(getValues(worldSST_rcp45), cid = 1:ncell(worldSST_rcp45)))
clim45[, c("x","y")] <- xyFromCell(worldSST_rcp45, clim45$cid)

# RCP 85
worldSST_rcp85 <- as(rast(list(hco, rcp85, hco_sd)), "Raster")
names(worldSST_rcp85) <- c("MnSST_HCO", "MnSST_RCP85", "sdSST_HCO")
clim85 <- na.omit(data.frame(getValues(worldSST_rcp85), cid = 1:ncell(worldSST_rcp85)))
clim85[, c("x","y")] <- xyFromCell(worldSST_rcp85, clim85$cid)

# climate velocity and distance to analogues under RCP 4.5
hco_ca45 <- dVoCC(clim45, n = 1, tdiff = 7140, method = "Variable", climTol = NA,
                  geoTol = Inf, distfun = "GreatCircle", trans = NA, lonlat = TRUE)
...
analogue_dist <- worldSST_rcp85[[c(1,1,1,1)]]
...
# Save to raster file
writeRaster(analogue_dist,
            file.path(outDir, "climate_analogue_distances_wgs84.grd"),
            overwrite = TRUE)
...
```{r plotDist, echo = TRUE, fig.show='asis', fig.cap="Distance to climate analogues under RCP 4.5 and RCP 8.5", fig.align='center', fig.height=10, fig.width=9, results='hide'}
...
rdf <- melt(setDT(as.data.frame(rdf, xy = TRUE, na.rm = TRUE)), id.vars = c("x", "y"))
rdf[, variable := factor(variable, levels = c("RCP45", "RCP85", "DeltaDist"))]

# projected shapefiles for aesthetics
hotspots_prj <- read_sf(file.path(outDir, "shapes/hotspots_prj.shp"))
hotspots_prj <- hotspots_prj[hotspots_prj$Thresh == 95, ]
land_prj <- read_sf(file.path(outDir, "shapes/land_prj.shp"))
regions_prj <-  read_sf(file.path(outDir, "shapes/regions_merged_prj.shp"))
coast_prj <- read_sf(file.path(outDir, "shapes/coast_prj.shp"))
bbox_prj <- read_sf(file.path(outDir, "shapes/bbox_prj.shp"))
...
figS3 <- ggplot() +
  ...
  scale_fill_gradientn(colours = cptcity::cpt("jjg_cbac_seq_cbacYlGnBu09", 100),
                       limits = c(0, 4500),
                       labels = function(x) ifelse(test = (seq(0, 4500, by = 500)/500) %% 2 == 0, x, ""),
...
ggsave(filename = "FigureS3.pdf", plot = figS3,
       device = cairo_pdf, path = file.path(outDir, "Figures"),
       width = 5, height = 6.5, units = "in", dpi = 350)

# save to PNG for MS
ggsave(filename = "FigureS3.png", plot = figS3,
       device = "png", type = "cairo", path = file.path(outDir, "Figures"),
...
```{r plotDistDelta, echo = TRUE, fig.show='asis', fig.cap="Differences between distances to climate analogues under RCP 4.5 and RCP 8.5", fig.align='center', fig.height=5, fig.width=9}
ggplot() +
  geom_raster(data = rdf[variable == "DeltaDist",], aes(x, y, fill = value)) +
  geom_sf(data = land_prj, size = 0.25,
          inherit.aes = FALSE, colour = NA, fill = "#E9E9E9") +
  ...
  scale_fill_gradientn(colours = cptcity::cpt("cmocean_curl", 100),
                       limits = c(-3000, 3000),
                       labels = function(x) ifelse(test = (seq(-3000, 3000, by = 500)/500) %% 2 == 0, x, ""),
...
hs_int <- st_transform(
  st_intersection(st_buffer(
    st_transform(hotspots, crs = "EPSG:32662"), 0),
...
regions_w_hs$RCP45Min <- round(extract(analogue_dist[["RCP45"]], y = vect(regions_w_hs),
                                 fun = min, na.rm = TRUE)[,2])
regions_w_hs$RCP45Max <- round(extract(analogue_dist[["RCP45"]], y = vect(regions_w_hs),
                                 fun = max, na.rm = TRUE)[,2])
regions_w_hs$RCP45Mean <- round(extract(analogue_dist[["RCP45"]], y = vect(regions_w_hs),
                                 fun = mean, na.rm = TRUE)[,2])
regions_w_hs$RCP85Min <- round(extract(analogue_dist[["RCP85"]], y = vect(regions_w_hs),
                                 fun = min, na.rm = TRUE)[,2])
regions_w_hs$RCP85Max <- round(extract(analogue_dist[["RCP85"]], y = vect(regions_w_hs),
                                 fun = max, na.rm = TRUE)[,2])
regions_w_hs$RCP85Mean <- round(extract(analogue_dist[["RCP85"]], y = vect(regions_w_hs),
                                 fun = mean, na.rm = TRUE)[,2])
...
```{r vectDisp, echo = TRUE, results='hide', fig.show='asis', out.width='100%', fig.align='center', fig.height=5, fig.width=9}
# Only keep points in hotspot locations
dif45sf <- st_as_sf(as.data.frame(Dif45), coords = c("XS", "YS"),
                    crs = "EPSG:4326")
dif45_int <- st_transform(st_as_sf(erase.point(y = as_Spatial(st_transform(dif45sf, crs = "EPSG:32662")),
...
vect45 <- st_transform(
  st_as_sf(gcIntermediate(dif45_int[, c("XS", "YS")],
                          dif45_int[, c("XE", "YE")],
                          n = 30, addStartEnd = TRUE, 
                          breakAtDateLine = TRUE, sp = TRUE)),
  crs = mollPrj)
vect85 <- st_transform(
  st_as_sf(gcIntermediate(dif85_int[, c("XS", "YS")],
                          dif85_int[, c("XE", "YE")],
                          n = 30, addStartEnd = TRUE, 
                          breakAtDateLine = TRUE, sp = TRUE)),
  crs = mollPrj)
...
figS4 <- ggplot() +
  ...
  scale_colour_manual(values = c("RCP45" = "#D6BE6983",
                                  "RCP85" = "#D66A5E83"),
...
# RCP 45
worldSST_rcp45_eez <- as(mask(rast(list(hco, rcp45, hco_sd)), mask = vect(eez)), "Raster")
names(worldSST_rcp45_eez) <- c("MnSST_HCO", "MnSST_RCP45", "sdSST_HCO")
clim45ns <- na.omit(data.frame(getValues(worldSST_rcp45_eez), cid = 1:ncell(worldSST_rcp45_eez)))
clim45ns[, c("x","y")] <- xyFromCell(worldSST_rcp45_eez, clim45ns$cid)

# RCP 85
worldSST_rcp85_eez <- as(mask(rast(list(hco, rcp85, hco_sd)), mask = vect(eez)), "Raster")
names(worldSST_rcp85_eez) <- c("MnSST_HCO", "MnSST_RCP85", "sdSST_HCO")
clim85ns <- na.omit(data.frame(getValues(worldSST_rcp85_eez), cid = 1:ncell(worldSST_rcp85_eez)))
clim85ns[, c("x","y")] <- xyFromCell(worldSST_rcp85_eez, clim85ns$cid)

# climate velocity and distance to analogues under RCP 4.5
## constrained to EEZ
hco_ca45_eez <- dVoCC(clim45ns, n = 1, tdiff = 7140, method = "Variable", climTol = NA,
                      geoTol = Inf, distfun = "GreatCircle", trans = NA, lonlat = TRUE)

# climate velocity and distance to analogues under RCP 8.5
## constrained to EEZ
hco_ca85_eez <- dVoCC(clim85ns, n = 1, tdiff = 7140, method = "Variable", climTol = NA,
                      geoTol = Inf, distfun = "GreatCircle", trans = NA, lonlat = TRUE)
...
writeRaster(analogue_dist_eez,
            file.path(outDir, "climate_analogue_distances_EEZ_wgs84.grd"),
            overwrite = TRUE)
...
```{r plotDistEEZ, echo = TRUE, fig.show='asis', fig.cap="Distance to climate analogues under RCP 4.5 and RCP 8.5 constrained to EEZ", fig.align='center', fig.height=10, fig.width=9, results='hide'}
...
# projected shapefiles for aesthetics
eez_prj <- read_sf(file.path(outDir, "shapes/eez_buffer.shp"))
...
figS5 <- ggplot() +
  ...
  scale_fill_gradientn(colours = cptcity::cpt("jjg_cbac_seq_cbacYlGnBu09", 100),
                       limits = c(0, 6500),
                       labels = function(x) ifelse(test = (seq(0, 6500, by = 500)/500) %% 2 == 0, x, ""),
...
```{r deltaDistEEZ, echo = TRUE, fig.show='asis', fig.cap="Differences between distances to climate analogues under RCP 4.5 and RCP 8.5", fig.align='center', fig.height=5, fig.width=9}
ggplot() +
  geom_tile(data = rdf[variable == "DeltaDist",], aes(x, y, fill = value)) +
  geom_sf(data = land_prj, size = 0.25,
          inherit.aes = FALSE, colour = NA, fill = "#E9E9E9") +
  ...
  scale_fill_gradientn(colours = cptcity::cpt("cmocean_curl", 100),
                       limits = c(-5000, 5000),
                       labels = function(x) ifelse(test = (seq(-5000, 5000, by = 1000)/1000) %% 2 == 0, x, ""),
...
# Rasterise the bioregions
bio_ras <- rasterize(vect(regions[1]), y = analogue_dist_eez[[1]],
                     field = "Realm")
# Regions
...
```{r vectDispEEZ, echo = TRUE, results='hide', fig.show='asis', out.width='100%', fig.align='center', fig.height=5, fig.width=9}
# Only keep points in hotspot locations
...
vect45eez$Scen <- "RCP45"
vect85eez$Scen <- "RCP85"

vectseez <- rbind(vect45eez, vect85eez)

# Projected lines from ArcGIS - issue with wrapping caused
# by pacific centered projection?
# try plot(vectseez); plot(st_transform(vectseez, "proj")) to see the issue

vectseez <- read_sf(file.path(outDir, "shapes/hs_eez_displacement.shp"))
...
figS6 <- ggplot() +
  ...
  geom_sf(data = vectseez, aes(colour = Scen), size = 1, #colour = "#D6BE6983",
          inherit.aes = FALSE, show.legend = TRUE) +
  scale_colour_manual(values = c("RCP45" = "#D6BE6983",
                                  "RCP85" = "#D66A5E83"),
                      labels = c("RCP45" = "RCP 4.5",
  ...
  theme(legend.position = "bottom",
        ...
        legend.direction = "horizontal")
# save to PDF
ggsave(filename = "FigureS6.pdf", plot = figS6,
       device = cairo_pdf, path = file.path(outDir, "Figures"),
       width = 7, height = 5, units = "in", dpi = 350)

# save to PNG for MS
ggsave(filename = "FigureS6.png", plot = figS6,
       device = "png", type = "cairo", path = file.path(outDir, "Figures"),
       bg = "white", width = 7, height = 5, units = "in", dpi = 350)

# plot
figS6 + labs(caption = "Figure S6 - Dispersal from hotspot locations to climate analogues under RCP 4.5 and RCP 8.5 restricted to EEZ")
```
