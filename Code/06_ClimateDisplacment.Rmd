...
ggplot() +
  ...
  facet_wrap(~variable, ncol = 2,
             labeller = as_labeller(c("HCO" = "Holocene thermal optimum",
                                    ...
                                    "RCP45" = "RCP 4.5",
                                    "RCP85" = "RCP 8.5"))) +
  scale_fill_gradientn(colours = cpt("oc_sst", 100),
...
figS4 <- ggplot() +
  ...
  geom_sf(data = hotspots_prj, size = 0.25,
          inherit.aes = FALSE, fill = NA, colour = "black") +
  geom_sf(data = vects, aes(colour = Scen), size = 1, 
...
ggplot() +
  ...
  geom_sf(data = bbox_prj, size = 0.25,
          inherit.aes = FALSE, fill = NA, colour = "black") +
  scale_fill_gradientn(colours = cpt("cmocean_curl", 100),
                       limits = c(-8000, 8000),
                       labels = function(x) ifelse(test = (seq(-8000, 8000, by = 1000)/4000) %% 2 %in% c(0,1), x, ""),
...
vectseez <- st_read("shapes/mapping_shapes.gpkg", layer = "eez_displ_prj",
                    quiet = TRUE)
vectseez$Scen <- factor(vectseez$Scen, levels = c("RCP45", "RCP85"))
vectseez$Distance <- st_length(vectseez)
units(vectseez$Distance) <- "km"
...
{fig04 <- ggplot(data = vectseez, aes(colour = Scen, group = Scen)) +
    facet_wrap(~Scen, ncol = 1,
               labeller = as_labeller(c("RCP45" = "a", "RCP85" = "b"))) +
    geom_sf(data = land_prj, size = 0.25,
            inherit.aes = FALSE, colour = NA, fill = "#E9E9E9") +
    ...
    geom_sf(data = eez_prj, size = 0.5,
          inherit.aes = FALSE, fill = NA, colour = "#91608D") +
    geom_sf(size = 1, show.legend = TRUE) +
    scale_colour_manual(values = c("RCP45" = "#f4a58283",
                                   "RCP85" = "#b2182b83"),
                        labels = c("RCP45" = "RCP 4.5",
                                   "RCP85" = "RCP 8.5"),
...
vectseez_dt <- setDT(st_drop_geometry(vectseez))
vectseez_dt <- vectseez_dt[, .(xMean = mean(as.numeric(Distance)),
                               xMed = median(as.numeric(Distance))), by = Scen]
dist_density <- ggplot() +
  geom_density(data = vectseez, 
               aes(x = as.numeric(Distance), y = ..scaled.., group = Scen,
                   colour = Scen, fill = Scen),
  ...
  scale_x_continuous(expand = c(0, 0), 
                     breaks = scales::pretty_breaks(n = 6)(seq(min(as.numeric(vectseez$Distance)), max(as.numeric(vectseez$Distance)), by = 500)),
                     labels = function(x) x/1000) +
  scale_y_continuous(expand = c(0, 0)) +
  geom_segment(data = vectseez_dt,
               aes(y = 0.01, yend = 1, x = xMean, xend = xMean,
                   colour = Scen, group = Scen), 
               show.legend = FALSE,
               inherit.aes = FALSE,
               arrow = grid::arrow(ends = "first", angle = 30, length = unit(0.5,"cm"))) +
  theme_half_open() +
  theme(
    legend.margin = margin(0, 0, 0, -0.5, unit = "lines"),
    strip.text.x = element_blank(),
    ...
    panel.spacing.x = unit(0.2, "lines"),
    plot.margin = unit(c(0,0,0,0), "lines")) +
  labs(x = expression("Distance (km " %*% " 1000)"),
       y = "density") +
  theme(legend.position = "none")

fig04_comp <- ggdraw(fig04 + theme(legend.position = "bottom")) +
 draw_plot(hco_search + 
              theme(plot.background = element_rect(fill = "#FFFFFF", colour = NA),
                    legend.margin = margin(0, -0.5, 0, -0.2, unit = "lines")),
            width = 0.40, height = 0.17, x = 0.01, y = 0.005) +
  draw_plot_label(label = "c", x = 0.01, y = 0.20, size = 14) +
  draw_plot(dist_density +
              theme(plot.background = element_rect(fill = "#FFFFFF", colour = NA),
                    axis.text.y = element_blank(),
                    axis.title.y = element_blank(),
                    axis.title.x = element_blank(),
                    axis.text.x = element_text(size = 8)),
            width = 0.40, height = 0.17, x = 0.59, y = 0.005) +
  draw_plot_label(label = "d", x = 0.90, y = 0.20, size = 14)
...
ggsave(filename = "Figure04.pdf", plot = fig04_comp,
       device = cairo_pdf, path = "Figures",
       family = "Helvetica",
       width = 6, height = 8, units = "in", dpi = 350)

# save to PNG for MS
ggsave(filename = "Figure04.png", plot = fig04_comp,
       device = "png", type = "cairo", path = "Figures",
       bg = "white", width = 6, height = 8, units = "in", dpi = 350)
