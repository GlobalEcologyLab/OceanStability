...
source(file.path(outDir, "source/bivariate_map.R"))
...
rx <- mask(rast(file.path(outDir, "outputs/snr_st_delta_rcp45.tif")), ocean_mask)

# Species richness data
ry <- rast(file.path(outDir, "outputs/marine_richness_rescaled_wgs84.tif"))
...
A more suitable classification may come from using Fishers clustering algorithm (@fisher1958, @wand1997).
...
fig01 <- fig01 +
  ...
  draw_plot_label(label = "e", x = 0.865, y = 0.30, size = 14)
fig01 <- fig01 +
  draw_plot(msr_dens +
              scale_y_continuous(expand = c(0.15,0.15)) +
               theme(plot.background = element_rect(fill = NA, colour = NA),
