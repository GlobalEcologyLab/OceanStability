...
hs_int <- st_transform(st_intersection(st_buffer(st_transform(hotspots, crs = "EPSG:32662"), 0),
...
ggplot() + 
  ...
  scale_fill_manual(values = {set.seed(18);sample(hcl.colors(12, "Dynamic"), 12, replace = FALSE)}) +
...
cell_snr <- fread(file.path(outDir, "SNR_Cell.csv"))
...
hotspot_delta_snr <- st_transform(st_intersection(
